...
if [[ $mode == "error" ]]; then
    ...
    if [ "$transB" -eq 0 -a "$OPID" -ne 43 ]; then
        mpirun -np $rank_size ./output/bin/${OPNAME}_0_0 "$M" "$N" "$K" "$IPPORT" 10 0
    elif [ "$transB" -eq 0 -a "$OPID" -eq 43 ]; then
        mpirun -np $rank_size ./output/bin/${OPNAME}_0_0 "$M" "$N" "$K" "$group_size" "$IPPORT" 10 0
    elif [ "$transB" -eq 1 -a "$OPID" -ne 43 ]; then
        mpirun -np $rank_size ./output/bin/${OPNAME}_0_1 "$M" "$N" "$K" "$IPPORT" 10 0
    elif [ "$transB" -eq 1 -a "$OPID" -eq 43 ]; then
        mpirun -np $rank_size ./output/bin/${OPNAME}_0_1 "$M" "$N" "$K" "$group_size" "$IPPORT" 10 0
    else
        echo "Error: Invalid transB value"
        exit 1
    fi
...
if [[ $mode == "prof" ]]; then
    ...
    if [ "$transB" -eq 0 -a "$OPID" -ne 43 ]; then
        mpirun -np $rank_size msprof --application="./output/bin/${OPNAME}_0_0 $M $N $K $IPPORT 10 1" --output="$output_dir_us" 
    elif [ "$transB" -eq 0 -a "$OPID" -eq 43 ]; then
        mpirun -np $rank_size msprof --application="./output/bin/${OPNAME}_0_0 $M $N $K $group_size $IPPORT 10 1 1" --output="$output_dir_us" 
    elif [ "$transB" -eq 1 -a "$OPID" -ne 43 ]; then
        mpirun -np $rank_size msprof --application="./output/bin/${OPNAME}_0_1 $M $N $K $IPPORT 10 1" --output="$output_dir_us" 
    elif [ "$transB" -eq 1 -a "$OPID" -eq 43 ]; then
        mpirun -np $rank_size msprof --application="./output/bin/${OPNAME}_0_1 $M $N $K $group_size $IPPORT 10 1" --output="$output_dir_us" 
    else
        echo "Error: Invalid transB value"
        exit 1
    fi
