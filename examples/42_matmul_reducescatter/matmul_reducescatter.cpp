...
#include "catlass/distributed/kernel/matmul_reducescatter.hpp"
...
static void Run(const Options &options) {
    ...
    LayoutTagB tagB{k, n};
    LayoutTagC tagC{m, n};
    LayoutTagC tagWC{m / rankSize, n};
    LayoutTagA tagWA{m / rankSize, k};
    ...
    // std::vector<int> signal(rankSize, 0);
    ...
    void *syncPtrBase = shmem_malloc((1024 * 1024));
    uint8_t *syncFlagPtr = (uint8_t *)syncPtrBase;  // 信号数组
    // ACL_CHECK(aclrtMemcpy(syncFlagPtr, rankSize * sizeof(int32_t), signal.data(), rankSize * sizeof(int32_t), ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    auto layoutWA = MakeLayoutFromTag(tagWA);
    ...
    using TensorWC = 
        Tensor<AscendC::GlobalTensor<ElementC>, decltype(layoutWC), tla::Coord<tla::_0, tla::_0>, AscendC::TPosition::GM>;

    constexpr bool enableUnitFlag = true;
    constexpr bool enableShuffleK = true;

    using MmadDispatchPolicy = Gemm::MmadAtlasA2Preload<enableUnitFlag, enableShuffleK>;
    using CommDispatchPolicy = Distributed::CommAtlasA2ReduceScatter;
    ...
    if (!isNeedPaddingA && !isNeedPaddingB) {
        ...
        if (options.problemShape.m() > options.problemShape.n()) {
            ...
            // kernel level
            using MatmulKernel = Distributed::Kernel::MatmulReduceScatterTla<BlockMmad, BlockEpilogue, BlockScheduler, PaddingA, PaddingB, BlockComm>;
            ...
            MatmulKernel::Arguments arguments{
                options.problemShape, L1Shape, options.commBlockShape, deviceA, layoutA, deviceB, layoutB, deviceC, layoutC, 
                deviceWA, layoutWA, deviceWB, layoutWB, symmtricPtr, syncFlagPtr, rankSize
            };
    ...
    // 从设备端拷贝结果
    std::vector<fp16_t> hostC(lenC / rankSize);
    uint32_t local_m = options.problemShape.m() / options.rankSize;
    size_t local_sizeC = static_cast<size_t>(local_m) * n * sizeof(fp16_t);
    int64_t rankId_local = shmem_my_pe();
    size_t offset_bytes = static_cast<size_t>((rankId_local + rankSize) % rankSize) * local_sizeC;
    ACL_CHECK(aclrtMemcpy(hostC.data(), local_sizeC, deviceC + offset_bytes, local_sizeC, ACL_MEMCPY_DEVICE_TO_HOST));

    // 保存数据
    std::string dataPath = "./examples/42_matmul_reducescatter/data/";
    ...
    saveData(resultFilePath, hostC.data(), sizeC / rankSize);
