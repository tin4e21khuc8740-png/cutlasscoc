...
//     }
// }
template<class ElementA, class ElementB, class ElementGolden>
void ComputeMatmul(
    ...
    for (uint32_t i = 0; i < M; ++i) {
        for (uint32_t j = 0; j < N; ++j) {
            ...
            for (uint32_t k = 0; k < K; ++k) {
                accumulator += static_cast<ElementGolden>(dataA[i * K + k]) * static_cast<ElementGolden>(dataB[k * N + j]);
            }
            dataGolden[i * N + j] = static_cast<ElementGolden>(accumulator);
        }
    }
}
